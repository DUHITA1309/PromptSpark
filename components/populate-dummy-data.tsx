...
interface PopulateDummyDataProps {
  onDataPopulated?: () => void
}

export default function PopulateDummyData({ onDataPopulated }: PopulateDummyDataProps) {
  const { user } = useAuth()
  ...
  const populateData = async () => {
    if (!user?.id) {
      toast({
        ...
        description: "Please log in to populate dummy data.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      for (let i = 0; i < dummyPrompts.length; i++) {
        ...
        setProgress(((i + 1) / dummyPrompts.length) * 100)
      ...
      toast({
        title: "Success!",
        description: "Added 50 dummy prompts to your library.",
      })
  ...
  return (
    <Card className="max-w-md mx-auto">
      <CardHeader>
        ...
        <CardDescription>Add 50 diverse sample prompts to explore the library features.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {!isPopulated ? (
          <>
            <div className="text-sm text-gray-600">
              This will add 50 professional prompts across all categories including:
              <ul className="mt-2 space-y-1 list-disc list-inside text-xs">
                <li>Marketing & Sales templates</li>
                <li>Development & Technical docs</li>
                <li>Creative writing prompts</li>
                <li>Business & Strategy frameworks</li>
                <li>Research & Analysis guides</li>
            ...
            <Button onClick={populateData} disabled={isPopulating} className="w-full">
              {isPopulating ? (
                <>
          ...
          <Alert className="border-green-200 bg-green-50">
            ...
            <AlertDescription className="text-green-800">
              <strong>Success!</strong> 50 sample prompts have been added to your library. You can now explore all the
